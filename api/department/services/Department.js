...
module.exports = {
  ...
  fetchAll: (params) => {
    ...
    return Department.query(function(qb) {
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = Department.forge(params).save(data);
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Department._attributes)
      .filter(attribute => attribute !== Department.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Department._attributes[attribute].type));
